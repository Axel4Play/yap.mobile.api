...
$html = preg_replace('~(<!--c1-->)</div>~', '\1', $html);
$html = preg_replace("~<div class='postcolor'>\s*<!--ec2-->~", '<!--ec2-->', $html);
...
if (FALSE !== $dom) {
    ...
    $db = new PDO('pgsql:host='.YAP_HOST.';port=5432;dbname='.YAP_BASE.';user='.YAP_USER.';password='.YAP_PASS);
    ...
    foreach($dom->find('form table[id^=p_row_]') as $key => $element) {
        ...
        $ava  = null;
        if (NULL !== $element->find('span.postdetails a img', 0)) {
            $ava  = $element->find('span.postdetails a img', 0)->src;
        }
        if (empty($ava) && NULL !== $element->find('tr', 1)->find('td', 0)->find('img', 0)) {
            $ava  = $element->find('tr', 1)->find('td', 0)->find('img', 0)->src;
        }
        ...
        $text = preg_replace('~<!--c1-->.*?<!--ec1-->(<br>|\s)*~', '<blockquote>', $text);
        $text = preg_replace('~<!--c2-->.*?<!--ec2-->(<br>|\s)*~', '</blockquote>', $text);
        ...
        $text = preg_replace_callback(
            '~<a href=["\'](http://www\.yaplakal\.com/go|/go|/click)/\?(.*?)["\'].*?>~',
            create_function(
                '$matches',
                'return urldecode("<a href=\"{$matches[2]}\">");'
            ),
            $text
        );
        ...
        $data[] = [
            ...
            'user_id'    => (int) $user_id,
            'user_name'  => (string) html_entity_decode($user, ENT_HTML5, 'UTF-8'), // html_entity_decode(iconv('CP1251', 'UTF-8', $user), ENT_HTML5, 'UTF-8'),
            'avatar'     => (string) $ava,
            'rating'     => (int) $rate,
            'text'       => (string) $text //iconv('CP1251', 'UTF-8', $text),
            'width'      => (int) $width,
            'height'     => (int) $height,
